...
export class EmployeeService {
    ...
    upload(data, userId)  {
        ...
        return this.httpClient.post<IEmployee>(this.baseUrl, data, {
          reportProgress: true,
          observe: 'events'
        }).pipe(map((event) => {
    ...
    public uploadFile(fileToUpload: File): Observable<IEmployee> {
        const _formData = new FormData();
        _formData.append('file', fileToUpload, fileToUpload.name);   
        return this.httpClient.post<IEmployee>(this.baseUrl,  _formData, {
            headers: new HttpHeaders({
                'Content-Type': 'application/json'
            })  
